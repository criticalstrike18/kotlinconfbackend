...
import org.jetbrains.kotlinconf.podcast.PodcastService
import org.jetbrains.kotlinconf.ui.components.TabItem
import org.jetbrains.kotlinconf.ui.components.TabsView
import org.jetbrains.kotlinconf.ui.podcast.PodcastContainer
import org.jetbrains.kotlinconf.ui.podcast.PodcastScreen
import org.jetbrains.kotlinconf.ui.welcome.WelcomeScreen


@Composable
fun MainScreen(service: ConferenceService, podcastService: PodcastService) {
    ...
    val playerState by podcastService.playerState.collectAsState()
    ...
    TabsView(
        ...
        TabItem(
            ...
            PodcastContainer(
                ...
                onPlayPause = { episode ->
                    if (playerState.isPlaying && playerState.currentEpisode?.id == episode.id) {
                        podcastService.pause()
                    } else {
                        podcastService.play(episode)
                    }
                },
                onStop = { podcastService.stop() },
                onSpeedChange = { speed -> podcastService.setSpeed(speed) },
                onBoostChange = { enabled -> podcastService.enableBoost(enabled) },
                onSeek = { position -> podcastService.seekTo(position) },
                service = service
            )
        },
    )
