...
import org.jetbrains.kotlinconf.CategoriesTable
import org.jetbrains.kotlinconf.ChannelFullData
...
import org.jetbrains.kotlinconf.RoomTable
import org.jetbrains.kotlinconf.Score
import org.jetbrains.kotlinconf.Session
import org.jetbrains.kotlinconf.SessionInfo
import org.jetbrains.kotlinconf.Speaker
import org.jetbrains.kotlinconf.SpeakerInfo
...
//            hikariConfig.validate()
    ...
    suspend fun storePodcastData(importRequest: PodcastImportRequest): Int =
        newSuspendedTransaction(Dispatchers.IO) {
            ...
            importRequest.categories.forEach { catName ->
                PodcastCategories.insert {
                    it[PodcastCategories.channelId] = channelId
                    it[name] = catName
                }
            }
    ...
    suspend fun getAllPodcastData(): List<ChannelFullData> = newSuspendedTransaction(Dispatchers.IO) {
        // 1. Grab all channels
        val channelRows = PodcastChannels
            .selectAll()
            .orderBy(PodcastChannels.id to SortOrder.ASC)
            .map { row ->
                row[PodcastChannels.id] to row
            }

        // 2. For each channel, find its categories
        // 3. For each channel, find its episodes
        // We'll accumulate a list of ChannelFullData objects
        channelRows.map { (channelId, channelRow) ->
            // categories for this channel
            val categoryList = PodcastCategories
                .selectAll().where { PodcastCategories.channelId eq channelId }
                .map { it[PodcastCategories.name] }

            // episodes for this channel
            val episodeList = PodcastEpisodes
                .selectAll().where { PodcastEpisodes.channelId eq channelId }
                .orderBy(PodcastEpisodes.pubDate to SortOrder.DESC)
                .map { ep ->
                    EpisodeData(
                        guid = ep[PodcastEpisodes.guid],
                        title = ep[PodcastEpisodes.title],
                        description = ep[PodcastEpisodes.description],
                        link = ep[PodcastEpisodes.link],
                        pubDate = ep[PodcastEpisodes.pubDate],  // or format the Instant as ISO-8601
                        duration = ep[PodcastEpisodes.duration],
                        explicit = ep[PodcastEpisodes.explicit],
                        imageUrl = ep[PodcastEpisodes.imageUrl],
                        mediaUrl = ep[PodcastEpisodes.mediaUrl],
                        mediaType = ep[PodcastEpisodes.mediaType],
                        mediaLength = ep[PodcastEpisodes.mediaLength]
                    )
                }

            // Build a ChannelFullData
            ChannelFullData(
                id = channelRow[PodcastChannels.id],
                ...
                lastBuildDate = channelRow[PodcastChannels.lastBuildDate].toString(), // convert the Instant to string
